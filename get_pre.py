...
def get_pre(to_pre, model_name):
    """加载模型，输入时序数据，返回预测结果
    :param to_pre: 待测的数据 [1, time_step * completion_percentage, input_size]

    :return: pred_tag_lists, [[x, y, z]]
    """
    print("读取的模型为", model_name)
    lstm_model = load_model('./train_model_saved2/' + model_name + '.pkl')
...
with pd.ExcelWriter('output.xlsx') as writer:
    for endIndex in range(20, 21):
        test_lists, test_tag_lists = load_data(25, 30, for_train=False, percentage=0.99)
        # data, tag = load_data(21, 38, for_train=False, percentage=0.99)
        ...
        pred_tag_lists = get_pre(test_lists, "lstm" + '1' + '-' + str(endIndex))
        # pred_tag_lists = get_pre(test_lists, "lstm" + str(endIndex))
        ...
        df = pd.DataFrame({
            ...
            'Y': test_tag_lists[:, 1],
            'Z': test_tag_lists[:, 2],
            'X_pre': pred_tag_lists[:, 0],
            'Y_pre': pred_tag_lists[:, 1],
            'Z_pre': pred_tag_lists[:, 2],
            'MSE': ans,
        })
        # 导出到 Excel 文件
        df.to_excel(writer, sheet_name=str(endIndex), index=False)
        ...
        all_error.append(np.mean(ans))
        all_x_error.append(np.mean(x_error))
        all_y_error.append(np.mean(y_error))
        all_z_error.append(np.mean(z_error))
        all_x_R2.append(r2_score(test_tag_lists_x, pred_tag_lists_x))
        all_y_R2.append(r2_score(test_tag_lists_y, pred_tag_lists_y))
        all_z_R2.append(r2_score(test_tag_lists_z, pred_tag_lists_z))
        # 画手的位置和要预测点
        for i in range(len(test_lists)):
            draw_finger(test_lists[i][:, 24], test_lists[i][:, 25], test_lists[i][:, 26], test_tag_lists[i])
